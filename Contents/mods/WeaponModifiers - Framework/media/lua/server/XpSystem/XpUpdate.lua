...
xpUpdate.onWeaponHitXp = function(owner, weapon, hitObject, damage)
	...
	if weaponModifier then
        if weaponModifier.statsMultipliers then
            if weaponModifier.statsMultipliers.experience then
                xpMultiplier =  weaponModifier.statsMultipliers.experience
                -- print("experience = "..weaponModifier.statsMultipliers.experience)
            -- else
                -- print("no experience multiplier found")
            end
        -- else
            -- print("no statsMultipliers found")
        end
    -- else
	...
	if owner:getLastHitCount() > 0 and not weapon:isRanged() then
        -- if
        -- print("xpMultiplier = ".. xpMultiplier)
		-- print("original xp : ".. exp)
		exp = exp * xpMultiplier;
		-- print("new xp : ".. exp)
