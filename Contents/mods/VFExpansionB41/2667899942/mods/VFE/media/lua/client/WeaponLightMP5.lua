local function WeaponLightBeam()
	...
	if attacker ~= nil then				
		...
		if attacker:isAiming() and attacker:getPrimaryHandItem() and (string.find(weapon:getType(), "MP5")) then
			weapon:setTorchCone(true)
				weapon:setLightDistance(10)
				weapon:setLightStrength(1.5)
				end
		if not attacker:isAiming() and attacker:getPrimaryHandItem() and (string.find(weapon:getType(), "MP5")) then
