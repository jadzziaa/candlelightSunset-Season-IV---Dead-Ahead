ISGarage3Door = ISBuildingObject:derive("ISGarage3Door");

function ISGarage3Door:create(x, y, z, north, sprite)
...
function ISGarage3Door:new(sprite, spriteIndex)
	local o = {};
	setmetatable(o, self);
	self.__index = self;
	o:init();
...
function ISGarage3Door:addDoorPart(x,y,z,north,sprite,index)
	local cell = getWorld():getCell();
...
function ISGarage3Door:overrideModData(spriteIndex)
...
function ISGarage3Door:getHealth()
	return 500 + buildUtil.getWoodHealth(self);
end

function ISGarage3Door:isValid(square)
	...
	-- I added a check for walls between each part in IsoDoor.ISGarage3DoorObstructed().
	if square:getBuilding() or squareA:getBuilding() or squareB:getBuilding() then return false; end
...
function ISGarage3Door:render(x, y, z, square)
...
function ISGarage3Door:getSquare2Pos(square, north)
	local x = square:getX()
	local y = square:getY()
	local z = square:getZ()
	if north then
...
function ISGarage3Door:getSquare3Pos(square, north)
	local x = square:getX()
	local y = square:getY()
	local z = square:getZ()
	if north then
