...
ISBuildMenu.barbedWire = 0;
ISBuildMenu.cheat = false or getDebug();
ISBuildMenu.woodWorkXp = 0;

if getActivatedMods():contains("IBM-EB") then require("ISUI/IBM-Barricades"); end
...
ISBuildMenu.doBuildMenu = function(player, context, worldobjects, test)
                ...
                if not ISBuildMenu.cheat then
                    for x=0,stage:getItemsToKeep():size()-1 do
                        ...
                        if item then
                            ...
                            if hasItem then
                                tooltip.description = tooltip.description .. " <RGB:0,1,0> " .. item:getName() .. " <LINE> ";
                            else
                        ...
                        if item then
                            if instanceof(item, "DrainableComboItem") then
                                ...
                                if nbOfItem >= tonumber(v) then
                                    tooltip.description = tooltip.description .. " <RGB:0,1,0> " .. item:getName() .. " " .. nbOfItem .. "/" .. v .. " <LINE> ";
                                else
	...
	if ISBuildMenu.haveSomethingtoBuild(player) then
		...
		if haveSomethingtoBuildWood(player) then
			...
			ISBuildMenu.buildIBMFloorMenu(subMenuFloor, floorOption, player);
			------------------ FURNITURE ------------------
			...
			------------------ OUTDOOR ------------------
			local outdoorOption = subMenu:addOption(getText("ContextMenu_Outdoor"), worldobjects, nil);
			local subMenuOutdoor = subMenu:getNew(subMenu);
			context:addSubMenu(outdoorOption, subMenuOutdoor);
			ISBuildMenu.buildOutdoorMenu(subMenuOutdoor, context, outdoorOption, player);
            ------------------ LIGHT SOURCES ------------------
			...
			ISBuildMenu.buildIBMLightMenu(subMenuLight, lightOption, player);
			------------------ MISC ------------------
...
function haveSomethingtoBuildWood(player)
	...
	elseif ISBuildMenu.canBuildLogWall(player) then
		-- no hammer required
	elseif ISBuildMenu.canBuildLogPillar(player) then
		-- no hammer required
	else
		return false
	end
	...
	ISBuildMenu.barbedWire = ISBuildMenu.countMaterial(player, "Base.BarbedWire")
	return true;
end
	...
	if not ISBuildMenu.cheat then
		...
		for x=0,stage:getItemsToKeep():size()-1 do
			...
			if item then
				...
				if hasItem then
					tooltipTxt = tooltipTxt .. " <RGB:0,1,0> " .. item:getName() .. " <LINE> ";
				else
		...
		for x,v in pairs(items) do
			...
			if item then
				if instanceof(item, "DrainableComboItem") then
					...
					if useLeft >= tonumber(v) then
						tooltipTxt = tooltipTxt .. " <RGB:0,1,0> " .. item:getName() .. " " .. useLeft .. "/" .. v .. " <LINE> ";
					else
					...
					if nbOfItem >= tonumber(v) then
						tooltipTxt = tooltipTxt .. " <RGB:0,1,0> " .. item:getName() .. " " .. nbOfItem .. "/" .. v .. " <LINE> ";
					else
		...
		for x,v in pairs(perks) do
			local perk = PerkFactory.getPerk(x);
			if playerObj:getPerkLevel(x) >= tonumber(v) then
				tooltipTxt = tooltipTxt .. " <RGB:0,1,0> " .. getText("IGUI_perks_" .. perk:getType():toString()) .. " " .. playerObj:getPerkLevel(x) .. "/" ..  v .. " <LINE>";
			else
		...
		if knownRecipe then
			tooltipTxt = tooltipTxt .. " <LINE> "
			if playerObj:isRecipeKnown(stage:getKnownRecipe()) then
				tooltipTxt = tooltipTxt .. " <RGB:0,1,0> " .. getText("Tooltip_vehicle_requireRecipe", getRecipeDisplayName(knownRecipe)) .. " <LINE>"
			else
...
ISBuildMenu.buildWallMenu = function(subMenu, option, player)
	...
	local tooltip2 = ISBuildMenu.canBuild(2,3,0,0,0,2, pillarOption, player);
	tooltip2:setName(getText("ContextMenu_Wooden_Pillar"));
	tooltip2.description = getText("Tooltip_craft_woodenPillarDesc") .. tooltip2.description;
	tooltip2:setTexture("walls_exterior_wooden_01_27");
    ...
    local tooltip3 = ISBuildMenu.canBuild(0,0,0,0,0,0, logOption, player);
    tooltip3:setName(getText("ContextMenu_Log_Wall"));
    local numLog = ISBuildMenu.countMaterial(player, "Base.Log")
	local logName = getItemNameFromFullType("Base.Log")
	local rippedName = getItemNameFromFullType("Base.RippedSheets")
	local twineName = getItemNameFromFullType("Base.Twine")
	local ropeName = getItemNameFromFullType("Base.Rope")
    if numLog < 4 then
        tooltip3.description = tooltip3.description .. " <RGB:1,0,0>" .. logName .. " " .. numLog .. "/4 <LINE> ";
		if not ISBuildMenu.cheat then
			logOption.onSelect = nil;
			logOption.notAvailable = true;
		end
    else
        tooltip3.description = tooltip3.description .. " <RGB:0,1,0>" .. logName .. " " .. numLog .. "/4 <LINE> ";
    end
    tooltip3:setTexture("carpentry_02_80");
    ...
    if numRippedSheets >= 4 then
        tooltip3.description = tooltip3.description .. " <RGB:0,1,0>" .. rippedName .. " " .. numRippedSheets .. "/4 ";
    elseif numTwine >= 4 then
        tooltip3.description = tooltip3.description .. " <RGB:0,1,0>" .. twineName .. " " .. numTwine .. "/4 ";
    elseif numRope >= 2 then
        tooltip3.description = tooltip3.description .. " <RGB:0,1,0>" .. ropeName .. " " .. numRope .. "/2 ";
    else
        tooltip3.description = tooltip3.description .. " <RGB:1,0,0>" .. rippedName .. " " .. numRippedSheets .. "/4 <LINE> " .. getText("ContextMenu_or") .. " " .. twineName .. " " .. numTwine .. "/4 <LINE> " .. getText("ContextMenu_or") .. " " .. ropeName .. " " .. numRope .. "/2 ";
        if not ISBuildMenu.cheat then
            logOption.onSelect = nil;
            logOption.notAvailable = true;
        end
    end
    tooltip3.description = getText("Tooltip_craft_wallLogDesc") .. tooltip3.description;
	
	local logPillarOption = subMenu:addOption(getText("ContextMenu_Log_Pillar"), worldobjects, ISBuildMenu.onLogPillar, player);
    local tooltip4 = ISBuildMenu.canBuild(0,0,0,0,0,0, logPillarOption, player);
    tooltip4:setName(getText("ContextMenu_Log_Pillar"));
    local numLog = ISBuildMenu.countMaterial(player, "Base.Log")
    if numLog < 1 then
        tooltip4.description = tooltip4.description .. " <RGB:1,0,0>" .. logName .. " " .. numLog .. "/1 <LINE> ";
        if not ISBuildMenu.cheat then
            logPillarOption.onSelect = nil;
            logPillarOption.notAvailable = true;
        end
    else
        tooltip4.description = tooltip4.description .. " <RGB:0,1,0>" .. logName .. " " .. numLog .. "/1 <LINE> ";
    end
    tooltip4:setTexture("carpentry_02_83");

    -- log pillar require either 2 ripped sheet, 2 twine or 1 rope
    local numRippedSheets = ISBuildMenu.countMaterial(player, "Base.RippedSheets") + ISBuildMenu.countMaterial(player, "Base.RippedSheetsDirty")
    local numTwine = ISBuildMenu.countMaterial(player, "Base.Twine")
    local numRope = ISBuildMenu.countMaterial(player, "Base.Rope")
    if numRippedSheets >= 2 then
        tooltip4.description = tooltip4.description .. " <RGB:0,1,0>" .. rippedName .. " " .. numRippedSheets .. "/2 ";
    elseif numTwine >= 2 then
        tooltip4.description = tooltip4.description .. " <RGB:0,1,0>" .. twineName .. " " .. numTwine .. "/2 ";
    elseif numRope >= 1 then
        tooltip4.description = tooltip4.description .. " <RGB:0,1,0>" .. ropeName .. " " .. numRope .. "/1 ";
    else
        tooltip4.description = tooltip4.description .. " <RGB:1,0,0>" .. rippedName .. " " .. numRippedSheets .. "/2 <LINE> " .. getText("ContextMenu_or") .. " " .. twineName .. " " .. numTwine .. "/2 <LINE> " .. getText("ContextMenu_or") .. " " .. ropeName .. " " .. numRope .. "/1 ";
        if not ISBuildMenu.cheat then
            logPillarOption.onSelect = nil;
            logPillarOption.notAvailable = true;
        end
    end
    tooltip4.description = getText("Tooltip_craft_pillarLogDesc") .. tooltip4.description;

    if wallOption.notAvailable and logOption.notAvailable and pillarOption.notAvailable and logPillarOption.notAvailable then
...
ISBuildMenu.buildDoorMenu = function(subMenu, option, player)
	...
	local doorsBigWoodenGate = subMenu:addOption(getText("ContextMenu_Wooden_Gate"), worldobjects, ISBuildMenu.onWoodenGate, square, sprite, player);
	local tooltip = ISBuildMenu.canBuild(6,6,2,1,0,4,doorsBigWoodenGate, player);
	...
	tooltip:setTexture("fixtures_doors_fences_01_4");
	ISBuildMenu.requireHammer(doorsBigWoodenGate)
	
	local doorsBigWoodenGate = subMenu:addOption(getText("ContextMenu_Wooden_Big_Gate"), worldobjects, ISBuildMenu.onBigWoodenGate, square, sprite, player);
	local tooltip = ISBuildMenu.canBuild(6,6,2,1,0,4,doorsBigWoodenGate, player);
	tooltip:setName(getText("ContextMenu_Wooden_Big_Gate"));
	tooltip.description = getText("Tooltip_craft_WoodenBigGateDesc") .. tooltip.description;
	tooltip:setTexture("fixtures_doors_fences_01_12");
	ISBuildMenu.requireHammer(doorsBigWoodenGate)
	
	local sprite = {};
	sprite.sprite = "fixtures_doors_fences_01_";

	local doorsDoubleDoor = subMenu:addOption(getText("ContextMenu_Double_Wooden_Door"), worldobjects, ISBuildMenu.onDoubleWoodenDoor, square, sprite, 104, player);
	local tooltip = ISBuildMenu.canBuild(12,12,4,2,0,6,doorsDoubleDoor, player);
	...
	ISBuildMenu.requireHammer(doorsDoubleDoor)
	
	if doorFrameOption.notAvailable and doorsOption.notAvailable and doorsDoubleDoor.notAvailable then
		option.notAvailable = true;
	end
end
...
ISBuildMenu.onWoodenGate = function(worldobjects, square, sprite, player)
	-- sprite, northsprite, openSprite, openNorthSprite
	local door = ISWoodenDoor:new("fixtures_doors_fences_01_4", "fixtures_doors_fences_01_5", "fixtures_doors_fences_01_6", "fixtures_doors_fences_01_7");
	door.canBarricade = false;
    door.modData["xp:Woodwork"] = 3;
	door.modData["need:Base.Plank"] = "3";
	door.modData["need:Base.Nails"] = "3";
	door.modData["need:Base.Hinge"] = "2";
	door.modData["need:Base.Doorknob"] = "1";
	door.dontNeedFrame = true;
	door.player = player
	door.completionSound = "BuildWoodenStructureLarge";
	getCell():setDrag(door, player);
end
...
ISBuildMenu.onBigWoodenGate = function(worldobjects, square, sprite, player)
	-- sprite, northsprite, openSprite, openNorthSprite
	local door = ISWoodenDoor:new("fixtures_doors_fences_01_12", "fixtures_doors_fences_01_13", "fixtures_doors_fences_01_14", "fixtures_doors_fences_01_15");
	door.canBarricade = true;
    door.modData["xp:Woodwork"] = 6;
	door.modData["need:Base.Plank"] = "6";
	door.modData["need:Base.Nails"] = "6";
	door.modData["need:Base.Hinge"] = "2";
	door.modData["need:Base.Doorknob"] = "1";
	door.dontNeedFrame = true;
	door.player = player
...
ISBuildMenu.buildIBMFloorMenu = function(subMenu, option, player)
	-- simple wooden floor
	...
	local floorBrownOption = subMenu:addOption(getText("ContextMenu_Brown_Wooden_Floor"), worldobjects, ISBuildMenu.onWoodenBrownFloor, square, "floors_interior_tilesandwood_01_44", player);
	local tooltip2 = ISBuildMenu.canBuild(1,1,0,0,0,2,floorBrownOption, player);
	tooltip2:setName(getText("ContextMenu_Brown_Wooden_Floor"));
	tooltip2.description = getText("Tooltip_craft_woodenBrownFloorDesc") .. tooltip2.description;
	tooltip2:setTexture("floors_interior_tilesandwood_01_44");
	ISBuildMenu.requireHammer(floorBrownOption)
	
	local floorLightBrownOption = subMenu:addOption(getText("ContextMenu_Light_Brown_Wooden_Floor"), worldobjects, ISBuildMenu.onWoodenLightBrownFloor, square, "floors_interior_tilesandwood_01_42", player);
	local tooltip3 = ISBuildMenu.canBuild(1,1,0,0,0,2,floorLightBrownOption, player);
	tooltip3:setName(getText("ContextMenu_Light_Brown_Wooden_Floor"));
	tooltip3.description = getText("Tooltip_craft_woodenLightBrownFloorDesc") .. tooltip3.description;
	tooltip3:setTexture("floors_interior_tilesandwood_01_42");
...
ISBuildMenu.onWoodenBrownFloor = function(worldobjects, square, sprite, player)
	-- sprite, northSprite
	local foor = ISWoodenFloor:new("floors_interior_tilesandwood_01_44", "floors_interior_tilesandwood_01_44")
	foor.modData["xp:Woodwork"] = 1;
	foor.modData["need:Base.Plank"] = "1";
	foor.modData["need:Base.Nails"] = "1";
	foor.completionSound = "BuildWoodenStructureMedium";
...
ISBuildMenu.onWoodenLightBrownFloor = function(worldobjects, square, sprite, player)
	-- sprite, northSprite
	local foor = ISWoodenFloor:new("floors_interior_tilesandwood_01_42", "floors_interior_tilesandwood_01_42")
	foor.modData["xp:Woodwork"] = 1;
	foor.modData["need:Base.Plank"] = "1";
	foor.modData["need:Base.Nails"] = "1";
	foor.player = player
...
ISBuildMenu.buildFurnitureMenu = function(subMenu, context, option, player)
	...
	local barElemOption = subMenuBar:addOption(getText("ContextMenu_Bar_Element"), worldobjects, ISBuildMenu.onBarElement, barElemSprite, player);
	local tooltip = ISBuildMenu.canBuild(4,4,0,0,0,5,barElemOption, player);
	...
	local barCornerOption = subMenuBar:addOption(getText("ContextMenu_Bar_Corner"), worldobjects, ISBuildMenu.onBarElement, barCornerSprite, player);
	local tooltip2 = ISBuildMenu.canBuild(4,4,0,0,0,5,barCornerOption, player);
	...
	local tooltip = ISBuildMenu.canBuild(4,4,0,0,0,7,barElem2Option, player);
	tooltip:setName(getText("ContextMenu_Bar_Element"));
	tooltip.description = getText("Tooltip_craft_barElementDesc") .. tooltip.description;
	...
	local tooltip2 = ISBuildMenu.canBuild(4,4,0,0,0,7,barCorner2Option, player);
	tooltip2:setName(getText("ContextMenu_Bar_Corner"));
	tooltip2.description = getText("Tooltip_craft_barElementDesc") .. tooltip2.description;
	tooltip2:setTexture(barCorner2Sprite.sprite);
	...
	if getActivatedMods():contains("IBM-EB") then ISBuildMenu.buildBarricadeMenu(subMenu, context, option, player) end
    ...
    local tooltip3 = ISBuildMenu.canBuild(6,4,0,0,0,4,bedOption, player);
    tooltip3:setName(getText("ContextMenu_Bed"));
    tooltip3.description = getText("Tooltip_craft_bedDesc") .. tooltip3.description;
	-- we add that we need a mattress too
	local mattress = ISBuildMenu.countMaterial(player, "Base.Mattress");
    if mattress < 1 then
        tooltip3.description = tooltip3.description .. " <RGB:1,0,0>" .. getItemNameFromFullType("Base.Mattress") .. " " .. mattress .. "/1 ";
        if not ISBuildMenu.cheat then
            bedOption.onSelect = nil;
            bedOption.notAvailable = true;
        end
    else
        tooltip3.description = tooltip3.description .. " <RGB:1,1,1>" .. getItemNameFromFullType("Base.Mattress") .. " " .. mattress .. "/1 ";
    end
    tooltip3:setTexture(bedSprite.northSprite1);
	ISBuildMenu.requireHammer(bedOption)
	
	-- add the dining submenu
	local diningOption = subMenu:addOption(getText("ContextMenu_Dining"), worldobjects, nil);
	local subMenuDining = subMenu:getNew(subMenu);
	context:addSubMenu(diningOption, subMenuDining);

	-- add all our table option
	local tableSprite = ISBuildMenu.getWoodenTableSprites(player);
	local smallTableOption = subMenuDining:addOption(getText("ContextMenu_Small_Table"), worldobjects, ISBuildMenu.onSmallWoodTable, square, tableSprite, player);
	local tooltip4 = ISBuildMenu.canBuild(5,4,0,0,0,3,smallTableOption, player);
	tooltip4:setName(getText("ContextMenu_Small_Table"));
	tooltip4.description = getText("Tooltip_craft_smallTableDesc") .. tooltip4.description;
	tooltip4:setTexture(tableSprite.sprite);
	ISBuildMenu.requireHammer(smallTableOption)

	local largeTableSprite = ISBuildMenu.getLargeWoodTableSprites(player);
	local largeTableOption = subMenuDining:addOption(getText("ContextMenu_Large_Table"), worldobjects, ISBuildMenu.onLargeWoodTable, square, largeTableSprite, player);
	local tooltip5 = ISBuildMenu.canBuild(6,4,0,0,0,4,largeTableOption, player);
	tooltip5:setName(getText("ContextMenu_Large_Table"));
	tooltip5.description = getText("Tooltip_craft_largeTableDesc") .. tooltip5.description;
	tooltip5:setTexture(largeTableSprite.sprite1);
	ISBuildMenu.requireHammer(largeTableOption)

	local drawerSprite = ISBuildMenu.getTableWithDrawerSprites(player);
	local drawerTableOption = subMenuDining:addOption(getText("ContextMenu_Table_with_Drawer"), worldobjects, ISBuildMenu.onSmallWoodTableWithDrawer, square, drawerSprite, player);
	local tooltip6 = ISBuildMenu.canBuild(5,4,0,0,0,5,drawerTableOption, player);
	-- we add that we need a Drawer too
	local drawer = ISBuildMenu.countMaterial(player, "Base.Drawer");
	if drawer < 1 then
		tooltip6.description = tooltip6.description .. " <RGB:1,0,0>" .. getItemNameFromFullType("Base.Drawer") .. " " .. drawer .. "/1 <LINE>";
		if not ISBuildMenu.cheat then
			drawerTableOption.onSelect = nil;
			drawerTableOption.notAvailable = true;
		end
	else
		tooltip6.description = tooltip6.description .. " <RGB:0,1,0>" .. getItemNameFromFullType("Base.Drawer") .. " " .. drawer .. "/1 <LINE>";
	end
	tooltip6:setName(getText("ContextMenu_Table_with_Drawer"));
	tooltip6.description = getText("Tooltip_craft_tableDrawerDesc") .. tooltip6.description;
	tooltip6:setTexture(drawerSprite.sprite);
	ISBuildMenu.requireHammer(drawerTableOption)
	
	-- now the chair
	local chairSprite = ISBuildMenu.getWoodenChairSprites(player);
	local chairOption = subMenuDining:addOption(getText("ContextMenu_Wooden_Chair"), worldobjects, ISBuildMenu.onWoodChair, square, chairSprite, player);
	local tooltip7 = ISBuildMenu.canBuild(5,4,0,0,0,2,chairOption, player);
	tooltip7:setName(getText("ContextMenu_Wooden_Chair"));
	tooltip7.description = getText("Tooltip_craft_woodenChairDesc") .. tooltip7.description;
	tooltip7:setTexture(chairSprite.northSprite);
	ISBuildMenu.requireHammer(chairOption)

    if smallTableOption.notAvailable and largeTableOption.notAvailable and drawerTableOption.notAvailable and chairOption.notAvailable then
        diningOption.notAvailable = true;
    end
		
	-- add the stairs submenu
	...
	local stairsLightBrownSprite = ISBuildMenu.getLightBrownStairsSprite();
	local stairsLightBrownOption = subMenuStairs:addOption(getText("ContextMenu_Stairs_LightBrown"), worldobjects, ISBuildMenu.onStairs, square, stairsLightBrownSprite, player);
	local tooltip8 = ISBuildMenu.canBuild(15,15,0,0,0,6,stairsLightBrownOption, player);
	tooltip8:setName(getText("ContextMenu_Stairs_LightBrown"));
	tooltip8.description = getText("Tooltip_craft_stairsDesc") .. tooltip8.description;
	tooltip8:setTexture(stairsLightBrownSprite.sprite1);
	ISBuildMenu.requireHammer(stairsLightBrownOption)
	
	local stairsBrownSprite = ISBuildMenu.getBrownStairsSprite();
	local stairsBrownOption = subMenuStairs:addOption(getText("ContextMenu_Stairs_Brown"), worldobjects, ISBuildMenu.onStairs, square, stairsBrownSprite, player);
	local tooltip8 = ISBuildMenu.canBuild(15,15,0,0,0,6,stairsBrownOption, player);
	tooltip8:setName(getText("ContextMenu_Stairs_Brown"));
	tooltip8.description = getText("Tooltip_craft_stairsDesc") .. tooltip8.description;
	tooltip8:setTexture(stairsBrownSprite.sprite1);
	ISBuildMenu.requireHammer(stairsBrownOption)
	
	local stairsDarkSprite = ISBuildMenu.getDarkStairsSprite();
	local stairsDarkOption = subMenuStairs:addOption(getText("ContextMenu_Stairs_Dark"), worldobjects, ISBuildMenu.onStairs, square, stairsDarkSprite, player);
	local tooltip8 = ISBuildMenu.canBuild(15,15,0,0,0,6,stairsDarkOption, player);
	tooltip8:setName(getText("ContextMenu_Stairs_Dark"));
	tooltip8.description = getText("Tooltip_craft_stairsDesc") .. tooltip8.description;
	tooltip8:setTexture(stairsDarkSprite.sprite1);
	ISBuildMenu.requireHammer(stairsDarkOption)
	
	if stairsLightBrownOption.notAvailable and stairsBrownOption.notAvailable and stairsDarkOption.notAvailable then
        stairsOption.notAvailable = true;
    end
	
	-- add the storage submenu
    ...
    local bookOption = subMenuStorage:addOption(getText("ContextMenu_Bookcase"), worldobjects, ISBuildMenu.onBookcase, square, bookSprite, player);
    local tooltip9 = ISBuildMenu.canBuild(5,4,0,0,0,5,bookOption, player);
    tooltip9:setName(getText("ContextMenu_Bookcase"));
    tooltip9.description = getText("Tooltip_craft_bookcaseDesc") .. tooltip9.description;
    tooltip9:setTexture(bookSprite.sprite);
	ISBuildMenu.requireHammer(bookOption)

    local book2Sprite = ISBuildMenu.getSmallBookcaseSprite(player);
    local book2Option = subMenuStorage:addOption(getText("ContextMenu_SmallBookcase"), worldobjects, ISBuildMenu.onSmallBookcase, square, book2Sprite, player);
    local tooltip10 = ISBuildMenu.canBuild(3,3,0,0,0,3,book2Option, player);
    tooltip10:setName(getText("ContextMenu_SmallBookcase"));
    tooltip10.description = getText("Tooltip_craft_smallBookcaseDesc") .. tooltip10.description;
    tooltip10:setTexture(book2Sprite.sprite);
	ISBuildMenu.requireHammer(book2Option)

    local shelveSprite = ISBuildMenu.getShelveSprite(player);
    local shelveOption = subMenuStorage:addOption(getText("ContextMenu_Shelves"), worldobjects, ISBuildMenu.onShelve, square, shelveSprite, player);
    local tooltip11 = ISBuildMenu.canBuild(1,2,0,0,0,2,shelveOption, player);
    tooltip11:setName(getText("ContextMenu_Shelves"));
    tooltip11.description = getText("Tooltip_craft_shelvesDesc") .. tooltip11.description;
    tooltip11:setTexture(shelveSprite.sprite);
	ISBuildMenu.requireHammer(shelveOption)

    local shelve2Sprite = ISBuildMenu.getDoubleShelveSprite(player);
    local shelve2Option = subMenuStorage:addOption(getText("ContextMenu_DoubleShelves"), worldobjects, ISBuildMenu.onDoubleShelve, square, shelve2Sprite, player);
    local tooltip12 = ISBuildMenu.canBuild(2,4,0,0,0,4,shelve2Option, player);
    tooltip12:setName(getText("ContextMenu_DoubleShelves"));
    tooltip12.description = getText("Tooltip_craft_doubleShelvesDesc") .. tooltip12.description;
    tooltip12:setTexture(shelve2Sprite.sprite);
	ISBuildMenu.requireHammer(shelve2Option)
	
	-- Half Crate option on floor
	local crateHalfSprite = ISBuildMenu.getWoodenCrateHalfSprites(player);
	local crateHalfOption = subMenuStorage:addOption(getText("ContextMenu_Half_Wooden_Crate"), worldobjects, ISBuildMenu.onHalfWoodenCrate, square, crateHalfSprite, player);
	local tooltip13 = ISBuildMenu.canBuild(2,2,0,0,0,1,crateHalfOption, player);
	tooltip13:setName(getText("ContextMenu_Half_Wooden_Crate"));
	tooltip13.description = getText("Tooltip_craft_woodenCrateDesc") .. tooltip13.description;
	tooltip13:setTexture(crateHalfSprite.sprite);
	ISBuildMenu.requireHammer(crateHalfOption)
	
	-- Crate option on floor
	local crateSprite = ISBuildMenu.getWoodenCrateSprites(player);
	local crateOption = subMenuStorage:addOption(getText("ContextMenu_Wooden_Crate"), worldobjects, ISBuildMenu.onWoodenCrate, square, crateSprite, player);
	local tooltip14 = ISBuildMenu.canBuild(6,6,0,0,0,3,crateOption, player);
	tooltip14:setName(getText("ContextMenu_Wooden_Crate"));
	tooltip14.description = getText("Tooltip_craft_woodenCrateDesc") .. tooltip14.description;
	tooltip14:setTexture(crateSprite.sprite);
	ISBuildMenu.requireHammer(crateOption)
	
	-- Crate option on floor
	local crateMilitarySprite = {};
	crateMilitarySprite.sprite = "military_crate_black_01_0_0";
	crateMilitarySprite.northSprite = "military_crate_black_01_1_0"
	
	local crateMilitaryOption = subMenuStorage:addOption(getText("ContextMenu_Military_Crate"), worldobjects, ISBuildMenu.onMilitaryCrate, square, crateMilitarySprite, player);
	local tooltip15 = ISBuildMenu.canBuild(10,10,0,0,0,10,crateMilitaryOption, player);
	tooltip15:setName(getText("ContextMenu_Military_Crate"));
	tooltip15.description = getText("Tooltip_craft_woodenCrateDesc") .. tooltip15.description;
	tooltip15:setTexture(crateMilitarySprite.sprite);
	ISBuildMenu.requireHammer(crateMilitaryOption)
		
	if bookOption.notAvailable and book2Option.notAvailable and shelveOption.notAvailable and shelve2Option.notAvailable and crateHalfOption.notAvailable and crateOption.notAvailable and crateMilitaryOption.notAvailable then
        storageOption.notAvailable = true;
    end
	
    if bedOption.notAvailable and stairsOption.notAvailable and storageOption.notAvailable and diningOption.notAvailable then
		if getActivatedMods():contains("IBM-EB") then
			if barricadeOption.notAvailable then option.notAvailable = true; end
		end
        option.notAvailable = true;
    end
end
...
ISBuildMenu.onHalfWoodenCrate = function(worldobjects, square, crateSprite, player)
	-- sprite, northSprite
...
ISBuildMenu.onWoodenCrate = function(worldobjects, square, crateSprite, player)
	...
	crate.canBeAlwaysPlaced = true;
	crate.containerType = "crate";
    crate.modData["xp:Woodwork"] = 6;
	crate.modData["need:Base.Plank"] = "6";
	crate.modData["need:Base.Nails"] = "6";
	crate:setEastSprite(crateSprite.eastSprite);
	crate.player = player
	crate.completionSound = "BuildWoodenStructureMedium";
	getCell():setDrag(crate, player);
...
ISBuildMenu.onMilitaryCrate = function(worldobjects, square, crateSprite, player)
	-- sprite, northSprite
	local crate = ISWoodenContainer:new(crateSprite.sprite, crateSprite.northSprite);
	crate.renderFloorHelper = true
	crate.canBeAlwaysPlaced = true;
	crate.containerType = "crate";
    crate.modData["xp:Woodwork"] = 10;
	crate.modData["need:Base.Plank"] = "10";
	crate.modData["need:Base.Nails"] = "10";
	crate.player = player
	crate.completionSound = "BuildWoodenStructureMedium";
	getCell():setDrag(crate, player);
end
...
-- ISBuildMenu.onMilitaryCrate = function(worldobjects, square, crateSprite, player)
	-- -- sprite, northSprite
	-- local crate = ISWoodenContainer:new(crateSprite.sprite);
	-- crate.name = "Military Crate";
	-- crate.renderFloorHelper = true
	-- crate.canBeAlwaysPlaced = true;
	-- crate.containerType = "militarycrate";
    -- crate.modData["xp:Woodwork"] = 10;
	-- crate.modData["need:Base.Plank"] = "10";
	-- crate.modData["need:Base.Nails"] = "10";
	-- crate.northSprite = crateSprite.sprite;
	-- crate:setEastSprite(crateSprite.eastSprite);
	-- crate.player = player
	-- crate.completionSound = "BuildWoodenStructureMedium";
	-- getCell():setDrag(crate, player);
...
ISBuildMenu.onStairs = function(worldobjects, square, sprite, player)
    local stairs = ISWoodenStairs:new(sprite.sprite1, sprite.sprite2, sprite.sprite3, sprite.northSprite1, sprite.northSprite2, sprite.northSprite3, sprite.pillar, sprite.pillarNorth);
...
-- **                  *OUTDOOR*                 **
-- **********************************************
ISBuildMenu.buildOutdoorMenu = function(subMenu, context, option, player)
	-- Create submenu for fence items
	...
	local stakeOption = subMenuFence:addOption(getText("ContextMenu_Wooden_Stake"), worldobjects, ISBuildMenu.onWoodenFenceStake, square, player);
	...
	local barbedOption = subMenuFence:addOption(getText("ContextMenu_Barbed_Fence"), worldobjects, ISBuildMenu.onBarbedFence, square, player);
	...
	local fenceWoodOption = subMenuFence:addOption(getText("ContextMenu_Wooden_Fence"), worldobjects, ISBuildMenu.onWoodenFence, square, woodenFenceSprite, player);
	local tooltip3 = ISBuildMenu.canBuild(2,3,0,0,0,2,fenceWoodOption, player);
	...
	ISBuildMenu.requireHammer(fenceWoodOption)

	local sandBagOption = subMenuFence:addOption(getText("ContextMenu_Sang_Bag_Wall"), worldobjects, ISBuildMenu.onSangBagWall, square, player);
	...
	if sandbag < 3 then
		...
		tooltip4.description = tooltip4.description .. " <RGB:0,1,0>" .. getItemNameFromFullType("Base.Sandbag") .. " " .. sandbag .. "/3 ";
	end
    ...
    local gravelBagOption = subMenuFence:addOption(getText("ContextMenu_Gravel_Bag_Wall"), worldobjects, ISBuildMenu.onGravelBagWall, square, player);
    ...
    if gravelbag < 3 then
        ...
        tooltip5.description = tooltip5.description .. " <RGB:0,1,0>" .. getItemNameFromFullType("Base.Gravelbag") .. " " .. gravelbag .. "/3 ";
    end
    ...
    if stakeOption.notAvailable and barbedOption.notAvailable and fenceWoodOption.notAvailable and sandBagOption.notAvailable and gravelBagOption.notAvailable then
        fenceOption.notAvailable = true;
    end
	
	-- Create submenu for collectors items
	local collectorOption = subMenu:addOption(getText("ContextMenu_Collector"), worldobjects, nil);
	local subMenuCollector = subMenu:getNew(subMenu);
	context:addSubMenu(collectorOption, subMenuCollector);
	
	-- compost
    local compostOption = subMenuCollector:addOption(getText("ContextMenu_Compost"), worldobjects, ISBuildMenu.onCompost, player, "camping_01_19");
    local tooltip6 = ISBuildMenu.canBuild(5,4,0,0,0,2,compostOption, player);
    tooltip6:setName(getText("ContextMenu_Compost"));
    tooltip6.description = getText("Tooltip_craft_compostDesc") .. tooltip6.description;
    tooltip6:setTexture("camping_01_19");
    ISBuildMenu.requireHammer(compostOption)
	
	-- small rain collector barrel
	local barrelRainSmallOption = subMenuCollector:addOption(getText("ContextMenu_Rain_Collector_Barrel"), worldobjects, ISBuildMenu.onCreateSmallBarrel, player, "carpentry_02_54", RainCollectorBarrel.smallWaterMax);
	local tooltip7 = ISBuildMenu.canBuild(4,4,0,0,0,4,barrelRainSmallOption, player);
	-- we add that we need 2 garbage bag too
	local garbagebag = ISBuildMenu.countMaterial(player, "Base.Garbagebag");
    if garbagebag < 2 then
        tooltip7.description = tooltip7.description .. " <RGB:1,0,0>" .. getItemNameFromFullType("Base.Garbagebag") .. " " .. garbagebag .. "/2 ";
        if not ISBuildMenu.cheat then
            barrelRainSmallOption.onSelect = nil;
            barrelRainSmallOption.notAvailable = true;
        end
    else
        tooltip7.description = tooltip7.description .. " <RGB:0,1,0>" .. getItemNameFromFullType("Base.Garbagebag") .. " " .. garbagebag .. "/2";
    end
	tooltip7:setName(getText("ContextMenu_Rain_Collector_Barrel"));
	tooltip7.description = getText("Tooltip_craft_rainBarrelDesc") .. tooltip7.description;
	tooltip7:setTexture("carpentry_02_54");
	ISBuildMenu.requireHammer(barrelRainSmallOption)

    -- large rain collector barrel
    local barrelRainLargeOption = subMenuCollector:addOption(getText("ContextMenu_Large_Rain_Collector_Barrel"), worldobjects, ISBuildMenu.onCreateLargeBarrel, player, "carpentry_02_52", RainCollectorBarrel.largeWaterMax);
    local tooltip8 = ISBuildMenu.canBuild(8,8,0,0,0,8,barrelRainLargeOption, player);
    tooltip8:setName(getText("ContextMenu_Large_Rain_Collector_Barrel"));
    tooltip8.description = getText("Tooltip_craft_rainBarrelDesc") .. tooltip8.description;
    tooltip8:setTexture("carpentry_02_52");
	ISBuildMenu.requireHammer(barrelRainLargeOption)
	
	if compostOption.notAvailable and barrelRainSmallOption.notAvailable and barrelRainLargeOption.notAvailable then
        collectorOption.notAvailable = true;
    end
	
	if fenceOption.notAvailable and collectorOption.notAvailable then
		option.notAvailable = true;
	end	
end
...
ISBuildMenu.onCreateSmallBarrel = function(worldobjects, player, sprite, waterMax)
    ...
    barrel.modData["need:Base.Garbagebag"] = "2";
...
-- **          *LIGHT SOURCES*                 **
-- **********************************************
ISBuildMenu.buildIBMLightMenu = function(subMenu, option, player)
	...
	local lightSource = ISBuildMenu.countMaterial(player, "Base.Torch")
	local rope = ISBuildMenu.countMaterial(player, "Base.Rope")
	if lightSource < 1 then
		toolTip.description = toolTip.description .. " <RGB:1,0,0>" .. getItemNameFromFullType("Base.Torch") .. " " .. lightSource .. "/1 <LINE> ";
		if not ISBuildMenu.cheat then
			lampOption.onSelect = nil;
			lampOption.notAvailable = true;
		end
    else
        toolTip.description = toolTip.description .. " <RGB:0,1,0>" .. getItemNameFromFullType("Base.Torch") .. " " .. lightSource .. "/1 <LINE> ";
    end
	if rope < 1 then
		toolTip.description = toolTip.description .. " <RGB:1,0,0>" .. getItemNameFromFullType("Base.Rope") .. " " .. rope .. "/1 <LINE> ";
		if not ISBuildMenu.cheat then
			lampOption.onSelect = nil;
			lampOption.notAvailable = true;
		end
    else
        toolTip.description = toolTip.description .. " <RGB:0,1,0>" .. getItemNameFromFullType("Base.Rope") .. " " .. rope .. "/1 <LINE> ";
    end
	...
	if lampOption.notAvailable then option.notAvailable = true; end
end
...
ISBuildMenu.buildMiscMenu = function(subMenu, option, player)
	...
	local tooltip1 = ISBuildMenu.canBuild(2,2,0,0,0,1,crossOption, player);
	tooltip1.description = getText("Tooltip_craft_woodenCrossDesc") .. tooltip1.description;
	tooltip1:setName(getText("ContextMenu_Wooden_Cross"));
	tooltip1:setTexture("location_community_cemetary_01_23");
    ...
    local tooltip2 = ISBuildMenu.canBuild(3,3,0,0,0,1,signOption, player);
	tooltip2.description = getText("Tooltip_craft_signDesc") .. tooltip2.description;
    tooltip2:setName(getText("ContextMenu_Sign"));
    tooltip2:setTexture(signSprite.sprite);
	...
	local tooltip3 = ISBuildMenu.canBuild(0,0,0,0,0,0,stonePileOption, player);
	tooltip3.description = getText("Tooltip_craft_stonePileDesc") .. tooltip3.description;
	-- we add that we need stone too
	local stones = getSpecificPlayer(player):getInventory():getItemCount("Base.Stone", true);
	if stones < 6 then
		tooltip3.description = tooltip3.description .. " <RGB:1,0,0>" .. getItemNameFromFullType("Base.Stone") .. " " .. stones .. "/6 ";
		if not ISBuildMenu.cheat then
			stonePileOption.onSelect = nil;
			stonePileOption.notAvailable = true;
		end
	else
		tooltip3.description = tooltip3.description .. " <RGB:0,1,0>" .. getItemNameFromFullType("Base.Stone") .. " " .. stones .. "/6 ";
	end
	tooltip3:setName(getText("ContextMenu_Stone_Pile"));
	tooltip3:setTexture("location_community_cemetary_01_30");
	...
	local tooltip4 = ISBuildMenu.canBuild(1,0,0,0,0,0,woodenPicketOption, player);
	tooltip4.description = getText("Tooltip_craft_woodenPicketDesc") .. tooltip4.description;
	local ropes = tonumber(getSpecificPlayer(player):getInventory():getItemCount("Base.SheetRope", true));
	-- we add that we need rope too
	if ropes == 0 then
		tooltip4.description = tooltip4.description .. " <RGB:1,0,0>" .. getItemNameFromFullType("Base.SheetRope") .. " " .. ropes .. "/1 ";
		if not ISBuildMenu.cheat then
			woodenPicketOption.onSelect = nil;
			woodenPicketOption.notAvailable = true;
		end
	else
		tooltip4.description = tooltip4.description .. " <RGB:0,1,0>" .. getItemNameFromFullType("Base.SheetRope") .. " " .. ropes .. "/1 ";
	end
	tooltip4:setName(getText("ContextMenu_Wooden_Picket"));
	tooltip4:setTexture("location_community_cemetary_01_31");
	
	if crossOption.notAvailable and signOption.notAvailable and stonePileOption.notAvailable and woodenPicketOption.notAvailable then
		option.notAvailable = true;
	end
end
...
-- name, sprite, northSprite
    ...
    furniture.blockAllTheSquare = false;
    furniture.isWallLike = true
    furniture.modData["xp:Woodwork"] = 1;
    furniture.modData["need:Base.Plank"] = "3";
    furniture.modData["need:Base.Nails"] = "3";
    furniture.player = player
    furniture.completionSound = "BuildWoodenStructureSmall";
...
-- Legacy Menu's --
ISBuildMenu.buildStairsMenu = function(subMenu, player) end
ISBuildMenu.buildFloorMenu = function(subMenu, player) end
ISBuildMenu.buildContainerMenu = function(subMenu, player) end
ISBuildMenu.buildLightMenu = function(subMenu, player) end

-- **********************************************
-- **            SPRITE FUNCTIONS              **
-- **********************************************

ISBuildMenu.getLightBrownStairsSprite = function()
    local sprite = {};
	sprite.sprite1 = "fixtures_stairs_01_19";
	sprite.sprite2 = "fixtures_stairs_01_20";
	sprite.sprite3 = "fixtures_stairs_01_21";
	sprite.northSprite1 = "fixtures_stairs_01_27";
	sprite.northSprite2 = "fixtures_stairs_01_28";
	sprite.northSprite3 = "fixtures_stairs_01_29";
	sprite.pillar = "fixtures_stairs_01_30";
	sprite.pillarNorth = "fixtures_stairs_01_31";
    return sprite;
end

ISBuildMenu.getBrownStairsSprite = function()
	-- local spriteLvl = ISBuildMenu.getSpriteLvl(player);
    local sprite = {};
	-- if spriteLvl == 2 then
		-- sprite.sprite1 = "carpentry_02_88";
		-- sprite.sprite2 = "carpentry_02_89";
		-- sprite.sprite3 = "carpentry_02_90";
		-- sprite.northSprite1 = "carpentry_02_96";
		-- sprite.northSprite2 = "carpentry_02_97";
		-- sprite.northSprite3 = "carpentry_02_98";
		-- sprite.pillar = "carpentry_02_94";
		-- sprite.pillarNorth = "carpentry_02_95";
	-- else
		sprite.sprite1 = "fixtures_stairs_01_32";
		sprite.sprite2 = "fixtures_stairs_01_33";
		sprite.sprite3 = "fixtures_stairs_01_34";
		sprite.northSprite1 = "fixtures_stairs_01_40";
		sprite.northSprite2 = "fixtures_stairs_01_41";
		sprite.northSprite3 = "fixtures_stairs_01_42";
		sprite.pillar = "fixtures_stairs_01_38";
		sprite.pillarNorth = "fixtures_stairs_01_39";
    return sprite;
end

ISBuildMenu.getDarkStairsSprite = function()
    local sprite = {};
	sprite.sprite1 = "fixtures_stairs_01_16";
	sprite.sprite2 = "fixtures_stairs_01_17";
	sprite.sprite3 = "fixtures_stairs_01_18";
	sprite.northSprite1 = "fixtures_stairs_01_24";
	sprite.northSprite2 = "fixtures_stairs_01_25";
	sprite.northSprite3 = "fixtures_stairs_01_26";
	sprite.pillar = "fixtures_stairs_01_22";
	sprite.pillarNorth = "fixtures_stairs_01_22";
    return sprite;
end
...
ISBuildMenu.getBedSprite = function(player)
	...
	elseif spriteLvl == 2 then
		...
		sprite.sprite1 = "furniture_bedding_01_3";
		sprite.sprite2 = "furniture_bedding_01_2";
		sprite.northSprite1 = "furniture_bedding_01_0";
		sprite.northSprite2 = "furniture_bedding_01_1";
	end
    return sprite;
end
...
ISBuildMenu.canBuild = function(plankNb, nailsNb, hingeNb, doorknobNb, barbedWireNb, carpentrySkill, option, player)
	-- create a new tooltip
	local tooltip = ISBuildMenu.addToolTip();
	-- add it to our current option
	option.toolTip = tooltip;
	local result = true;
	tooltip.description = "<LINE> <LINE>" .. getText("Tooltip_craft_Needs") .. ":";
	tooltip.description = tooltip.description .. " <LINE> ";
	if getSpecificPlayer(player):getPerkLevel(Perks.Woodwork) < carpentrySkill then
		tooltip.description = tooltip.description .. " <RGB:1,0,0>" .. getText("IGUI_perks_Carpentry") .. " " .. getSpecificPlayer(player):getPerkLevel(Perks.Woodwork) .. "/" .. carpentrySkill .. " <LINE><LINE>";
		result = false;
	elseif carpentrySkill > 0 then
		tooltip.description = tooltip.description .. " <RGB:0,1,0>" .. getText("IGUI_perks_Carpentry") .. " " .. getSpecificPlayer(player):getPerkLevel(Perks.Woodwork) .. "/" .. carpentrySkill .. " <LINE><LINE>";
	end
	...
	if ISBuildMenu.barbedWire < barbedWireNb then
		tooltip.description = tooltip.description .. " <RGB:1,0,0>" .. getItemNameFromFullType("Base.BarbedWire") .. " " .. ISBuildMenu.barbedWire .. "/" .. barbedWireNb .. " <LINE>";
		result = false;
	elseif barbedWireNb > 0 then
		tooltip.description = tooltip.description .. " <RGB:0,1,0>" .. getItemNameFromFullType("Base.BarbedWire") .. " " .. ISBuildMenu.barbedWire .. "/" .. barbedWireNb .. " <LINE>";
	end
	if ISBuildMenu.cheat then
		return tooltip;
	end
    ...
    -- -- we add that we need a mattress too
	-- local mattress = ISBuildMenu.countMaterial(getSpecificPlayer(player), "Base.Mattress");
    -- if mattress < 1 then
        -- tooltip3.description = tooltip3.description .. " <RGB:1,0,0>" .. getItemNameFromFullType("Base.Mattress") .. " " .. mattress .. "/1 ";
        -- if not ISBuildMenu.cheat then
            -- bedOption.onSelect = nil;
        ...
        -- end
    -- else
        -- tooltip3.description = tooltip3.description .. " <RGB:0,1,0>" .. getItemNameFromFullType("Base.Mattress") .. " " .. mattress .. "/1 ";
    -- end
