...
buildUtil.getWoodHealth = function(ISItem)
	if not ISItem or not ISItem.player then return 100; end
	local playerObj = getSpecificPlayer(ISItem.player)
	local health = (playerObj:getPerkLevel(Perks.Woodwork) * 50);
	if ISItem.modData and ISItem.modData["use:Base.BlowTorch"] then health = (playerObj:getPerkLevel(Perks.MetalWelding) * 50); end
	if playerObj:HasTrait("Handy") then health = health + 100; end
	return health;
end
