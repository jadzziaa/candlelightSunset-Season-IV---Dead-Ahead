...
ISMMBurnTreatment = ISBaseTimedAction:derive("ISMMBurnTreatment");

function ISMMBurnTreatment:isValid()
...
function ISMMBurnTreatment:waitToStart()
...
function ISMMBurnTreatment:update()
    if self.character ~= self.otherPlayer then
        self.character:faceThisObject(self.otherPlayer)
    end
...
function ISMMBurnTreatment:start()
...
function ISMMBurnTreatment:stop()
    ISHealthPanel.setBodyPartActionForPlayer(self.otherPlayer, self.bodyPart, nil, nil, nil)
    ISBaseTimedAction.stop(self);
    self.item:setJobDelta(0.0);
end

function ISMMBurnTreatment:perform()
    self.item:setJobDelta(0.0);
    ...
    if not self.character:getModData()["mmBurnTreatment"] then
        self.bodyPart:setBurnTime(self.bodyPart:getBurnTime() - 15);
        self.character:getModData()["mmBurnTreatment"] = 11
    end
    self.item:Use();
...
function ISMMBurnTreatment:new(doctor, otherPlayer, item, bodyPart)
	local o = {}
	setmetatable(o, self)
	self.__index = self
	o.character = doctor;
