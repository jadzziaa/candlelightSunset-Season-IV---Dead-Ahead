...
local sliceKeyTable = {}
local sliceFuncTableOutside = {}
local sliceKeyTableOutside = {}
...
function VehicleMenuAPI.registerSliceOutside(sliceName, sliceFunction)
	if not sliceFuncTableOutside[sliceName] then
		table.insert(sliceKeyTableOutside, sliceName)
	end
	sliceFuncTableOutside[sliceName] = sliceFunction
end

function VehicleMenuAPI.unregisterSliceOutside(sliceName)
	for i,v in ipairs(sliceKeyTableOutside) do
		if v == sliceName then
			table.remove(sliceKeyTableOutside, i)
			break
		end
	end
	sliceFuncTableOutside[sliceName] = nil
end
...
function ISVehicleMenu.showRadialMenu(playerObj)
	...
	for _,k in ipairs(sliceKeyTable) do
		sliceFuncTable[k](menu, playerObj, vehicle)
	end
...
function ISVehicleMenu.showRadialMenuOutside(playerObj)
	...
	if vehicle then
		for _,k in ipairs(sliceKeyTableOutside) do
			sliceFuncTableOutside[k](menu, playerObj, vehicle)
		end
	end
...
VehicleMenuAPI.registerSlice("Sleep", function(menu, player, vehicle)
	if (not isClient() or getServerOptions():getBoolean("SleepAllowed")) then
		local doSleep = true;
		if player:getStats():getFatigue() <= 0.3 then
		...
		elseif vehicle:getCurrentSpeedKmHour() > 1 or vehicle:getCurrentSpeedKmHour() < -1 then
			...
			if player:getSleepingTabletEffect() < 2000 then
				...
				elseif (player:getHoursSurvived() - player:getLastHourSleeped()) <= 1 then
